...
def plotSlice(fname, step):
    ...
    # mask = (0.27 < z) & (z < 0.35)
    # mask = (-0.05 < z) & (z < 0.05)
    masked_arr = np.ma.masked_array(z, mask)

    print(np.ma.max(masked_arr))
    print(np.ma.min(masked_arr))
    ...
    im = ax.scatter(x[mask], y[mask], c=rho[mask], s=0.1, cmap=cm, vmin=-0.8, vmax=0.8, label=plabel)
    fig.colorbar(im)
